...
**字段说明：**
- batch_size：可选，不提供则自动反推
- 单机卡数：默认 8 卡
- 场景类型：RL、数据生产、Agent轨迹生成、低QPS、在线推理、RAG、编程辅助、chat、多轮对话等
- 目标：TPS优先 或 时延优先（TTFT/TPOT）

**重要**：先分析用户的输入，如果缺少以下信息，必须逐项提示用户输入或选择跳过。**不得跳过此步骤直接进入下一步**。

### 1. 模型信息
**请告诉我是哪个模型？**
- 模型名称/路径: （如 Qwen-72B、Kimi-K2.5 或本地权重路径）

**重要**：当用户提供模型名称（如 "Kimi-K2.5"）时，**不要**自己去网上查询或猜测模型参数（如参数量、层数等）。直接将模型名称传递给 llm-mem-estimator，它会自动从 HuggingFace 或本地缓存获取所需信息。
...
完成四优先级决策后，检查是否有**两个或以上优先级给出不同 PD 策略结论**。
