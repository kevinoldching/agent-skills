...
|---|------|----------|
| 0.5 | 完全平衡 | 无特殊倾向 |
| 0.3~0.7 | 基本平衡 | 支持混部（imbalance < 0.8） |
| <0.3 或 >0.7 | 失衡倾向 | 若 imbalance >= 0.8 则支持分离 |
...
- TP/EP 取值：1 或 单机卡数×N（N=1,2,3,...）；TP 在 [1, 单机卡数] 范围内必须为2的幂（1, 2, 4, 8, ...）；超出单机卡数后为单机卡数×N；EP=1 仅限非MoE模型；MoE模型 EP >= 单机卡数
- 总卡数必须是单机卡数的整数倍
...
3. **对每个 EP 值，枚举所有有效 TP**：
   - 遍历 TP/EP 值：[1, 单机卡数] 范围内为 1, 2, 4, 8, ...（2的幂）；超出单机卡数后为 单机卡数×2, 单机卡数×3, ...；仅当 TP 能整除 EP 且 TP/EP 在候选列表中时有效
   - 计算 DP = EP / TP
...
6. 总卡数 <= 可用卡数
7. TP/EP 取值：1 或 单机卡数×N（N=1,2,3,...）；TP 在 [1, 单机卡数] 范围内必须为2的幂（1, 2, 4, 8, ...）；超出单机卡数后为单机卡数×N；EP=1 仅限非MoE模型；MoE模型 EP >= 单机卡数
