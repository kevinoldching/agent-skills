...
| 单样本 KV Cache (2560 tokens) | 2560 × 2.5 MB | **~6.25 MB/sample** |
...
| TP × DP <= 实例总卡数 | 见下方 | 8×1=8 <= 8 | 1×56=56 <= 56 |
| 非MoE模型 | EP=1 or EP=TP | EP=1 | EP=1 |
| TP/EP/DP 为2的幂 | 1,2,4,8,16,32 | 8,1,1 ✓ | 1,1,56 ⚠️ (DP例外) |
...
|------|-----|------|
| 实例数 | 1 | 单大型decode实例 |
| 每实例卡数 | 56 | TP=1, EP=1, DP=56 |
| 总卡数 | 56 | |
| Tensor Parallel | 1 | 内存带宽密集型，TP=1最优 |
| Expert Parallel | 1 | Dense模型 |
| Data Parallel | 56 | 高并发decode |
...
|-------|------|----|----|----|--------|--------|
| Prefill | 16 | 8 | 1 | 2 | 2 | ~2M tokens/s |
| Decode | 48 | 1 | 1 | 48 | 1 | ~43K tokens/s |
| **合计** | **64** | - | - | - | 3 | **~43K tokens/s** |

- 优点: Prefill实例冗余，HA保障，prefill吞吐更高
- 缺点: Decode吞吐降至43K，**不满足50K目标**

#### 配置2: 1P1D 均衡 (推荐)

| Stage | 卡数 | TP | EP | DP | 实例数 | 吞吐量 |
|-------|------|----|----|----|--------|--------|
| Prefill | 8 | 8 | 1 | 1 | 1 | ~1M tokens/s |
| Decode | 56 | 1 | 1 | 56 | 1 | ~50K+ tokens/s |
...
| 配置 | 卡数 | TP | EP | DP | 吞吐量 |
|------|------|----|----|----|--------|
| PD Mixed | 64 | 8 | 1 | 8 | ~40K tokens/s |
...
**实际配置下:**
- Decode: 56卡, TP=1, DP=56
- 每卡吞吐: 50K / 56 ≈ 893 tokens/s per DP copy
- 每卡batch: 32 / 56 ≈ 0.57 samples per card → 实际需要轮询
...
```
每卡权重: 144 GB / 56 ≈ 2.57 GB (DP分片)
KV Cache (B=32, S=4096):
  - 每卡: 32 × 4096 × 2.5 MB / 56 ≈ 5.7 GB
每卡总占用: 2.57 + 5.7 ≈ 8.3 GB < 80 GB ✓
```

### 7.3 EP分片下的Decode配置验证

| 配置 | TP=1 | EP=1 | DP=56 | 总卡数 |
|------|------|------|-------|--------|
| 每卡权重 | 144/56≈2.57 GB | - | - | 2.57 GB |
| 每卡KV Cache | 32×4096×2.5MB/56 | ≈ 5.7 GB | - | 5.7 GB |
| 每卡总占用 | - | - | - | ~8.3 GB |
| 可用空间 | - | - | - | ~71.7 GB余量 |
...
- [x] TP=8 <= 单机卡数=8
- [x] TP × DP=8 <= Prefill总卡数=8
- [x] TP × DP=56 <= Decode总卡数=56
- [x] 非MoE模型: EP=1
- [x] TP/EP/DP 为2的幂: TP=8, EP=1, DP=1或56
...
| `decode_cards = TP_decode × DP_decode` | 1 × 56 = 56 |
...
| 模型 | Qwen-72B |
| 总GPU数 | 64 |
| Prefill实例 | 1 × 8卡 (TP8/EP1/DP1) |
| Decode实例 | 1 × 56卡 (TP1/EP1/DP56) |
| 目标吞吐 | 50,000 tokens/s |
| 预估TTFT | 50-100 ms |
| 预估TPOT | 10-20 ms |
| KV Cache/卡 | ~5.7 GB (Decode阶段) |
| 权重/卡 | ~2.57 GB (Decode, DP分片) |
